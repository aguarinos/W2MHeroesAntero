...
export class EditComponent {
  ...
  ngOnInit(){
    ...
    this.superHeroService.getSuperHeroeById(id).subscribe((data)=>{
      this.heroData = data
  ...
  onSubmit() {
    if(this.editHeroForm.valid){
      const updateFormData : Superhero = {...this.editHeroForm.value, id:parseInt(this.router.snapshot.paramMap.get("id") || "")}
     this.subscription= this.superHeroService.updateSuperHero(updateFormData).subscribe(()=>{
